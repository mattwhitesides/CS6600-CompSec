...
\subsubsection{Confidentiality}
Using NF to verify a breach in confidentiality, we need to trace the output of commands from the EC's point of view, then compare that with trace with the higher-level commands removed from the process and determine if we have the same output. In our case, the EC modifies the FRS and adds a new FR into the system. The question is, will the system still produce the same output with the higher-level commands purged.
...
The question becomes, is it indistinguishable if the FR is written by the MC or is due to something within the External Contractor’s domain. We showed in NI that the output of the EC and MC can alter the FR however one FR is not inhearently at a different level than another FR or indicates where it came from. Thus there's not particular differece in output of the folloiwing command sequesences.\\\\
