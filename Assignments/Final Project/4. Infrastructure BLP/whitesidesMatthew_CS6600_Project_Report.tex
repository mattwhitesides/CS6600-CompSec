...
\end{lstlisting}  
  ...
  However, when running \textit{upload\_flight\_record}, the BA will find they do not have ``own'' rights over the record, which is required so that they will go back to the original MC and as to grant them execute privileges to the FRS system. 
...
\subsubsection{Integrity}
  ...
  In an attempt at an integrity leak, our BA may seek to modify existing data and, to get permission to do so, may attempt the following sequence. 
...
Figure 3 shows the state after the \textbf{BA} leads the \textbf{MC} into giving them \textit{t} rights over \textbf{FR} they can exploit the system to take our laked rights to \textbf{FED} through the \textbf{FRS}. 
This would be established after the command \textit{grant\_flight\_record\_access(MC, BA, FR)}.
...
Therefore allowing BA to execute \textit{update\_flight\_record\_system(BA, FR)} which leaks access to the FED as shown by the state in Figure 4.
...
\subsection{BLP}
The Bell-LaPadula Model (BLM) focuses on establishing access control confidentiality through security levels and categories. BLM focuses on two simple rules in a security policy within a given ACM. 
...
For our BLM implementation, we first define the security classifications for our subjects and objects. Given we are dealing with military records, it makes sense to use the standard government security levels. In addition to the security level, subjects and objects will fall into categories based upon the appropriate work unit.  
...
\begin{table}[]
  ...
  \textbf{Clearance Level}        & \textbf{Subjects} & \textbf{Objects} \\
  \hline
  Flight Operations (FO)          & PC,IP         & ANC,RO          \\
  Maintenance Operations (MO)     & PC,IP,MC,FDA  & FED,RTD,FRS    \\
  Flight Data Operations (FDO)    & PC,IP,FDA,EC  & FED,FRS             
  \end{tabular}
\end{table}
