...
\begin{table*}[]
  ...
  \centering
  \begin{tabular}{lll}
  1.  & $\sim fr$ = true                 & FR has been modified by the EC.                                                       \\
  2.  & $w \vDash V_{fr}^{1}(w) = true$  & FRS cannot verify the FR has been modified.                                 \\
  3.  & $I_{2,1}fr$                      & FRS allows upload of the FR from the EC.                                                \\
  4.  & $B_1I_{2,1} \sim fr$             & FRS believes the FR from the EC.                                     \\
  5.  & $T_{2,1} \sim fr$                & FRS trusts the FR from the EC.                                                          \\
  6.  & $B_1I_{2,1} \sim fr \bigwedge T_{2,1} \sim fr \rightarrow B_1 \sim fr$ & FRS believes the FR in the FRS.                                             \\
  7.  & $I_{3,1}fr$                      & FRS logs everything is correct for the MC.                                             \\
  8.  & $B_3I_{3,1}fr$                   & MC belives the FRS logs.                                                    \\
  9.  & $T_{3,1}fr$                      & MC trusts the FR.                                                           \\
  10. & $B_3I_{3,1}fr \bigwedge T_{3,1}fr \rightarrow B_3 \sim fr$   & MC belives the FRS logs are correct but the FR is modified.                 \\
  11. & $w \vDash V_{fr}^{3}(w) = true$  & The verification procedure for FR's uploaded to the FRS always return true. \\
  12. & $MSDND (FRS) = \exists w \in W \vdash [(S_{fr} \; {XOR} \; S_{\sim fr})] \wedge [w \vdash \nexists (V_{\sim fr}^3 (w) \wedge \nexists V_{fr}^3 (w))]$ & 
  \end{tabular}}
\end{table*}
